...
class LightOpenID
{
    ...
    , $data
    , $oauth = array()
    , $curl_time_out = 35          // in seconds
    , $curl_connect_time_out = 32; // in seconds
    ...
    function __construct($host,$server="https://steamcommunity.com/openid/login",$identity="http://steamcommunity.com/openid",$identifier_select=true,$version=2, $proxy = null)
    {
        $this->set_version($version);
        $this->set_identity($identity);
        $this->set_server($server);
        $this->set_identifier_select($identifier_select);
        $this->set_realm($host);
...
//        }
    ...
    protected function set_server($server){
        $this->server = $server;
    }

    protected function set_identifier_select($identifier_select){
        $this->identifier_select = $identifier_select;
    }
    ...
    protected function set_identity($identity){
    ...
    protected function request_curl($url, $method='GET', $params=array(), $update_claimed_id)
    {
        ...
        if (!empty($this->proxy)) {
            curl_setopt($curl, CURLOPT_INTERFACE, $this->proxy);
...
//            }
    ...
    function validate()
    {
        ...
        if (!$this->server)
            $this->discover($this->claimed_id);
        else $server = $this->server;
